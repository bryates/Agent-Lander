...
def test_ready(mem):
    ...
    for _ in range(BATCH_SIZE):
        mem.store(np.zeros(STATE_SIZE), np.zeros(STATE_SIZE), 0, 0, False)
    assert not mem.ready()
    mem.store(np.zeros(STATE_SIZE), np.zeros(STATE_SIZE), 0, 0, False)
    assert mem.ready()
...
def test_store_and_sample(mem):
    ...
    next_state = np.arange(STATE_SIZE, dtype=np.float32) + 1
...
def test_circular_buffer():
    ...
    for i in range(OVERWRITE_MEM_SIZE + 2):
        mem.store(np.ones(STATE_SIZE) * i, np.ones(STATE_SIZE) * (i+1), i, i, False)
    assert mem.mem_ptr == OVERWRITE_MEM_SIZE + 2
    # The oldest experiences should be overwritten
    expected_oldest = np.ones(STATE_SIZE) * (OVERWRITE_MEM_SIZE)
    expected_second = np.ones(STATE_SIZE) * (OVERWRITE_MEM_SIZE + 1)
    assert np.all(mem.curr_state_memory[0] == expected_oldest)
    assert np.all(mem.curr_state_memory[1] == expected_second)
