...
def test_create_model(request):
    ...
    agent = model.DQNAgent(state_size=8, action_size=4, lr=LEARNING_RATE, gamma=GAMMA,
...
def test_load_model(request):
    ...
    if not agent:
        assert False, 'Could not find model!'
    agent_new = model.DQNAgent(state_size=8, action_size=4, lr=LEARNING_RATE, gamma=GAMMA,
                               epsilon_start=EPSILON_START, epsilon_end=EPSILON_END, epsilon_decay=EPSILON_DECAY)
